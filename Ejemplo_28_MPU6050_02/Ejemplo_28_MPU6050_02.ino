...
void loop()
{
  ...
  if (Serial.available()) {    //Si el puerto serial tiene mas de un dato disponible
  byteRead = Serial.read();  //Leer el dato recibido y guardarlo en la variable byteRead
  if(byteRead == 'a'){
  Serial.write((accel_t_gyro.value.x_accel+32768)/256);
  Serial.write(((accel_t_gyro.value.x_accel+32768)&0xFF)); 
  
  Serial.write((accel_t_gyro.value.y_accel+32768)/256);
  Serial.write(((accel_t_gyro.value.y_accel+32768)&0xFF));
  
  Serial.write((accel_t_gyro.value.z_accel+32768)/256);
  Serial.write(((accel_t_gyro.value.z_accel+32768)&0xFF));
